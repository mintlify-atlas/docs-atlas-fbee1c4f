...
description: Graph and tree traversal exploring nodes level by level
---
...
<Warning>
This implementation uses a [Queue](/dsa/queue) data structure, so you must implement it beforehand.
</Warning>
...
```js title="Graph BFS (adjacency matrix)"
...
```js title="Graph BFS (adjacency list)"
const adjList = {
  A: ['B'],
  B: ['A', 'C', 'E'],
  C: ['B', 'D'],
