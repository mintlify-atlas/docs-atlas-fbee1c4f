---
title: Branches
description: Learn how to work with Git branches, merging, and cherry-picking
---
...
In a Git repository, there is a default branch (usually called `main` or `master`). Beyond that, you can create other branches to develop new features, fix bugs, test ideas, or work on updates without affecting the main codebase.
...
<Tip>
You can even do this manually by creating a file and writing a commit hash inside it. The file name becomes the branch name, and it starts from the commit you specified.
</Tip>
...
This is the normal state where `HEAD` points to a branch reference. Any new commits you make are added to that branch, and `HEAD` moves forward along with it.

#### Detached HEAD

This occurs when `HEAD` points directly to a specific commit hash instead of a branch reference. In this state, new commits aren't associated with any branch, and you might need to create a new branch to save your work.
...
- **Without conflicts:** Git will carry your changes to the new branch with you.
- **With conflicts:** Git will block the branch switch. You'll be asked to commit, stash, or discard your changes before switching.
</Note>
...
![three-way](./three-way.png)

### Resolving Merge Conflicts

<Steps>
  <Step title="Identify conflicts">
    Git reports a merge conflict and shows which files are affected.
  </Step>
  
  <Step title="Review conflict markers">
    The conflicted files contain special markers showing differences between branches.
  ...
  <Step title="Edit the files">
    Edit the files to decide which changes to keep. You can choose one version or combine parts of both.
  </Step>
  ...
  <Step title="Complete the merge">
    Create a commit to finish the merge.
  </Step>
</Steps>
...
<Warning>
Cherry-picking creates new commits with different commit IDs. This can lead to duplicate commits if the process is not managed carefully and the branches are merged later.
</Warning>
