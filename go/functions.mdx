---
title: Functions
description: Master Go functions including multiple return values, variadic parameters, closures, and the defer keyword
---
...
<Info>Variadic functions accept zero or more arguments of the same type, which are treated as a slice inside the function.</Info>
...
<Info>First-class functions are functions that can be assigned to variables, passed as arguments to other functions, and returned as values from other functions.</Info>
...
<Info>Anonymous functions are functions without a name.</Info>
...
<Note>Fully independent types: Booleans, Numerics, Arrays, Structs.</Note>
...
<Note>Reference-containing types: Strings (immutable), Slices, Maps, Channels, Pointers, Functions, Interfaces.</Note>
...
<Info>Each call to `sum()` returns a new closure with its own independent variables (`total`), allowing `add1` and `add2` to maintain separate internal state.</Info>
...
<Tip>Multiple deferred statements are executed in last-in-first-out (LIFO) order. This means that the most recently deferred function is executed first.</Tip>