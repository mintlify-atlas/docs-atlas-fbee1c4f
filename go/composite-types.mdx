...
description: Work with Go's composite data structures including arrays, slices, maps, and structs
---
...
<Info>The `index: value` syntax assigns values to selected indices. Unspecified indices remain zero-valued.</Info>
...
<Info>Slicing selects a range of elements from an array.</Info>
...
<Info>The optional second return value is a boolean indicating whether the key was found.</Info>
...
<Note>Tags use backticks and the format: `key:"value"`, and are not limited to `json`.</Note>